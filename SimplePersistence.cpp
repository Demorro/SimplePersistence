...
const std::string SimplePersistence::PERSISTENCE_FILE_PATH = ".\\SimplePersistenceFileDirectory";
//Define Default Persistence File Name
const std::string SimplePersistence::PERSISTENCE_FILE_NAME = "PersistenceFile";
...
//Saves a Bool into the active persistence object.
void SimplePersistence::SetBool(std::string key, bool value)
{
	...
	if(HasEntry(key,boolEntryNode))
	{
		...
		pugi::xml_attribute boolTypeToModify = boolEntryNode.attribute("type");
		boolTypeToModify = "Bool";
	}
	else
	{
...
void SimplePersistence::SetInt(std::string key, int value)
{
	...
	if(HasEntry(key,intEntryNode))
	{
		...
		pugi::xml_attribute intTypeToModify = intEntryNode.attribute("type");
		intTypeToModify = "Int";
	}
	else
	{
...
//Saves a Float into the active persistence object.
void SimplePersistence::SetFloat(std::string key, float value)
{
	...
	if(HasEntry(key,floatEntryNode))
	{
		...
		pugi::xml_attribute floatTypeToModify = floatEntryNode.attribute("type");
		floatTypeToModify = "Float";
...
//Saves a Double into the active persistence object.
void SimplePersistence::SetDouble(std::string key, double value)
{
	//Check if the key already exists in the current persistance object
	pugi::xml_node doubleEntryNode;

	if(HasEntry(key,doubleEntryNode))
	{
		//The entry already exists, just modify the value
		pugi::xml_attribute doubleValueToModify = doubleEntryNode.attribute("value");
		doubleValueToModify = value;

		pugi::xml_attribute doubleTypeToModify = doubleEntryNode.attribute("type");
		doubleTypeToModify = "Double";
	...
	else
	{
		...
		// param node for the entry
		pugi::xml_node doubleEntry = currentPersistenceObject.append_child(key.c_str());

		// add attributes to param node
		//intEntry.append_attribute("key") = key.c_str();
		doubleEntry.append_attribute("type") = "Double";
		doubleEntry.append_attribute("value") = value;
	}

	
}

//Saves a String into the active persistence object.
void SimplePersistence::SetString(std::string key, std::string value)
{
	...
	if(HasEntry(key,stringEntryNode))
	{
		...
		pugi::xml_attribute stringTypeToModify = stringEntryNode.attribute("type");
		stringTypeToModify = "String";
...
//Gets the value of an Double of name key in active persistence object, will fail if entry dosent exist or isnt an Double
double SimplePersistence::GetDouble(std::string key)
{
	//EntryIsType uses HasEntry, so you're going to get three cascading error messages here if it fails, you can just turn them off though.

	//EntryIsType checks for HasEntry(), no need to worry here
	if(EntryIsType(PersistenceType::Double,key))
	{
		//If the key type is an float, then we can return it
		pugi::xml_node doubleEntryNode;
		doubleEntryNode = currentPersistenceObject.child(key.c_str());
		return doubleEntryNode.attribute("value").as_double();
	}
	else
	{
		//Its not an float, SO YOU CANT FUCKING RETURN IT GODDAMN IT ROSS!
		if(DEBUG_MODE)
		{
			std::cout << "ERROR : Entry is not an double or does not exist, returning 0" << std::endl;
		}
		return 0.0;
	}
}
...
std::string SimplePersistence::StringFromPersistenceType(PersistenceType type)
{
	...
	std::string  EnumStrings[] = { "Bool", "Int", "Float", "Double", "String" };
